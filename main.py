    ...
    while True:
        try:
           # === КОД ДЛЯ ПОДНЯТИЯ ЛОТОВ ===
            ...
            # Перехватываем ошибки (например, пропал интернет), чтобы скрипт не упал
            print(f"⚠️ Ошибка в функции auto_bump_lots: {e}")
        
        # Обязательная пауза перед следующим поднятием (например, 2 часа = 7200 секунд)
        # На FunPay кулдаун на поднятие лотов зависит от категории, укажи нужное время!
